function solution(input: string[]): number {
  ...
  let giantString: any = input.join("-");
  
  // An array to store all of the calibration values
  let calibrationValueArray: any = [];
  
  // An array to store the first number and last number of an input string
  let tempArr: any = [];
  ...
  for (let i = 0; i < giantString.length; i++) {
    ...
    // If the element at 'i' is a '-'
    if (giantString[i] === "-") {  
      // Only go forward if there's more than one element in the temp array   
      if (tempArr.length >= 2) {
  ...
  console.log(calibrationValueArray, totalSum);
